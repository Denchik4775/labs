...
def load_and_clean_data(data_dir="vhi_data"):
    # Отримуємо шлях до поточної директорії, де лежить app.py
    current_dir = os.path.dirname(os.path.abspath(__file__))
    target_path = os.path.join(current_dir, data_dir)
    
    # ДІАГНОСТИКА: виведемо в консоль, де саме ми шукаємо
    print(f"DEBUG: Шукаю файли в папці: {target_path}")
    
    files = glob.glob(os.path.join(target_path, "*.csv"))
    print(f"DEBUG: Знайдено файлів: {len(files)}")
    ...
    # Мапінг областей (як було раніше)
...
if df.empty:
    st.error("Дані не знайдено! Перевірте, чи існує папка 'vhi_data' з файлами.")
    st.stop()

# Інтерфейс
st.sidebar.header("Параметри аналізу")
selected_index = st.sidebar.selectbox("Індекс", ['VCI', 'TCI', 'VHI'])
...
with tab2:
    st.line_chart(filtered_df.set_index('Week')[selected_index])

with tab3:
    fig, ax = plt.subplots(figsize=(10, 5))
    mean_vals = df[df['Year'].between(*years)].groupby('Province_Name')[selected_index].mean().reset_index()
    colors = ['red' if n == selected_province else 'lightgray' for n in mean_vals['Province_Name']]
    sns.barplot(data=mean_vals, x='Province_Name', y=selected_index, palette=colors, ax=ax)
